...
const SideBar = ({ name, role }) => {
	...
	return (
		<>
			{state.logout && (
				<Dialog open={state.logout} action={logoutMutation.mutate} close={closeLogout}>
					<h1>Logout</h1>
			...
			<div className="w-[350px] h-[100vh] sticky top-0 bg-gray shadow-md p-[1rem] flex align-center justify-between flex-col">
				...
				<div className="grid gap-[1rem] mb-[1.6rem]">
					<button
						onClick={openLogout}
						className="text-red-500 text-start p-2 cursor-pointer hover:text-red-800 hover:scale-[1.1] transition"
					>
